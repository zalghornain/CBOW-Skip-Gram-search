...
while loop == True:
    ...
    for x in range(len(testdata)):
        percobaan += 1
        onehotencodekatatarget = dictionarykata[testdata[x]]
        hiddenLayer = np.dot(np.transpose(weightinputmatrix),onehotencodekatatarget)
        uj = np.dot(np.transpose(weightoutputmatrix),hiddenLayer)
        expuj = np.exp(uj - np.max(uj))
        ycj = expuj/np.sum(expuj)
        if x > 0 and x < len(testdata)-1:
            onehotencodeminsatu = dictionarykata[testdata[x-1]]
            onehotencodeplussatu = dictionarykata[testdata[x+1]]
            #cek kalo bagian kanannya bener duluan
            if np.argmax(ycj) == np.argmax(onehotencodeplussatu):
                test = ycj
                test[np.argmax(test)] = 0
                #baru cek bagian kirinya
                if np.argmax(ycj) == np.argmax(onehotencodeminsatu):
                    kebenaran += 1
            #cek kalo bagian kirinya bener
            if np.argmax(ycj) == np.argmax(onehotencodeminsatu):
                test = ycj
                test[np.argmax(test)] = 0
                #baru cek bagian kirinya
    ...
    if akurasi > 0.99 or startAwalTime >= 10800:
        loop == False
